...
class WordRep(nn.Module):
    def __init__(self, opt):
        ...
        self.embedding = nn.Embedding(21128, opt.embedding_dim, padding_idx=0)
        self.feature_size = opt.embedding_dim

        self.embed_drop = nn.Dropout(p=opt.dropout_rate)

        self.conv_dict = {1: [self.feature_size, 128],
                     2: [self.feature_size, 100, 128],
                     3: [self.feature_size, 150, 100, 128],
                     4: [self.feature_size, 200, 150, 100, 128]
                     }
...
class Model(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.word_embedding = WordRep(opt)
        ...
        for filter_size in filter_sizes:
            ...
            tmp = nn.Conv1d(self.word_embedding.feature_size, self.word_embedding.feature_size, kernel_size=filter_size,
                            padding=int(floor(filter_size / 2)))
            xavier_uniform(tmp.weight)
            one_channel.add_module('baseconv', tmp)

            conv_dimension = self.word_embedding.conv_dict[1]
            for idx in range(1):
        ...
        self.output_layer = OutputLayer(opt, self.filter_num * 128)
