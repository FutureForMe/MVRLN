...
class Model(nn.Module, ABC):
    ...
    def __init__(self, opt):
        ...
        # 模型第一路(2)双向LSTM
        self.bi_lstm = nn.LSTM(opt.embedding_dim, opt.embedding_dim//2, 1,bidirectional=True, batch_first=True)
        ...
        self.gc1 = GraphConvolution(3*self.embedding_dim, 3*self.embedding_dim)
        self.gc2 = GraphConvolution(3*self.embedding_dim, 3*self.embedding_dim)

        # 分类平面
        self.classifier = nn.Linear(3*self.embedding_dim, self.class_num)
    ...
    def forward(self, X, masks,entities,label_matrix,ent_label_matrix):
        ...
        doc_level_label_embed = doc_level_embed.unsqueeze(1).repeat(1,self.class_num,1)

        fuse_features = torch.cat((char_level_label_embed,entity_level_label_embed,doc_level_label_embed),dim=2)
