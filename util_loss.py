...
def L2loss_func(model,alpha):
    l2_loss = []
    for name,parma in model.named_parameters():
        if 'bias' not in name and 'word_embedding' not in name:
            l2_loss.append(0.5*alpha * torch.sum(torch.pow(parma,2)))
    return torch.mean(torch.stack(l2_loss))